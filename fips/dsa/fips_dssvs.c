...
static int
parse_mod (char *line, int *pdsa2, int *pL, int *pN, const EVP_MD ** pmd)
{
    char lbuf[10240];
    char *keyword, *value;
...
pqg (FILE * in, FILE * out)
{
    ...
    enum pqtype
    { PQG_NONE, PQG_PQ, PQG_G, PQG_GCANON }
    pqg_type = PQG_NONE;
            ...
            if (idx >= 0)
            {
                if (BN_cmp (dsa->g, g))
                    ...
                    fprintf (out, "Result = P" RESP_EOL);
                     ...
                     (!part_test && ((BN_cmp (dsa->g, g) || (counter != counter2)
                                      || (h != h2)))))
                fprintf (out, "Result = F" RESP_EOL);
            else
                fprintf (out, "Result = P" RESP_EOL);
...
static int
dss_paramcheck (int L, int N, BIGNUM * p, BIGNUM * q, BIGNUM * g,
                BN_CTX * ctx)
{
    BIGNUM *rem = NULL;
    if (BN_num_bits (p) != L)
...
#ifdef FIPS_ALGVS
int
fips_dssvs_main (int argc, char **argv)
#else
int
main (int argc, char **argv)
#endif
