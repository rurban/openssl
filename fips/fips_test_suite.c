...
FIPS_rsa_test (int bad)
{
    ...
    if (!FIPS_rsa_sign
            (key, input_ptext, sizeof (input_ptext) - 1, EVP_sha256 (),
             RSA_PKCS1_PADDING, 0, NULL, buf, &slen))
        goto end;

    r =
        FIPS_rsa_verify (key, input_ptext, sizeof (input_ptext) - 1,
                         EVP_sha256 (), RSA_PKCS1_PADDING, 0, NULL, buf, slen);
end:
    if (key)
        FIPS_rsa_free (key);
    if (r != 1)
...
FIPS_sha1_test ()
{
    ...
    {
        0x11, 0xf1, 0x9a, 0x3a, 0xec, 0x1a, 0x1e, 0x8e, 0x65, 0xd4, 0x9a, 0x38,
    ...
    ERR_clear_error ();
    if (!FIPS_digest (str, sizeof (str) - 1, md, NULL, EVP_sha1 ()))
        return 0;
...
FIPS_sha256_test ()
{
    ...
    ERR_clear_error ();
    if (!FIPS_digest (str, sizeof (str) - 1, md, NULL, EVP_sha256 ()))
        return 0;
    if (memcmp (md, digest, sizeof (md)))
        return 0;
    return 1;
...
FIPS_sha512_test ()
{
    ...
    ERR_clear_error ();
    if (!FIPS_digest (str, sizeof (str) - 1, md, NULL, EVP_sha512 ()))
        return 0;
    if (memcmp (md, digest, sizeof (md)))
        return 0;
    return 1;
...
static size_t
drbg_test_cb (DRBG_CTX * ctx, unsigned char **pout,
...
static size_t
drbg_fail_cb (DRBG_CTX * ctx, unsigned char **pout,
              int entropy, size_t min_len, size_t max_len)
{
    return 0;
}
...
static int
do_drbg_test (int type, int flags)
{
    DRBG_CTX *dctx;
    int rv = 0;
    size_t i;
    ...
    for (i = 0; i < 32; i++)
    {
        if (!FIPS_drbg_generate (dctx, randout, sizeof (randout), 0, NULL, 0))
            goto err;
        if (!FIPS_drbg_generate
                (dctx, randout, sizeof (randout), 0, dummy_drbg_entropy, 1))
            goto err;
    }
    rv = 1;
err:
...
static void
test_msg (const char *msg, int result)
{
...
static int
post_cb (int op, int id, int subid, void *ex)
{
    ...
    case FIPS_TEST_DRBG:
        ...
        {
            sprintf (asctmp, "%s %s",
                     lookup_id (subid >> 16), lookup_id (subid & 0xFFFF));
            exstr = asctmp;
        }
        else
            exstr = lookup_id (subid);
    ...
    if (fail_id == id && (fail_key == -1 || fail_key == keytype)
...
static int
do_fail_all (int fullpost, int fullerr)
{
    fail_list *ftmp;
    int rv;
    size_t i;
...
#ifdef FIPS_ALGVS
int
fips_test_suite_main (int argc, char **argv)
#else
int
main (int argc, char **argv)
#endif
{
     ...
     */
    ERR_clear_error ();
    test_msg ("2. Automatic power-up self test",
              FIPS_module_mode_set (1, pass));
    if (!FIPS_module_mode ())
        return 1;
    if (do_drbg_stick)
        FIPS_drbg_stick (1);
