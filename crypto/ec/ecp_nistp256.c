...
smallfelem_is_zero (const smallfelem small)
{
    ...
    is_p = (small[0] ^ kPrime[0]) |
           (small[1] ^ kPrime[1]) | (small[2] ^ kPrime[2]) | (small[3] ^ kPrime[3]);
    is_p--;
    is_p &= is_p << 32;
    is_p &= is_p << 16;
    is_p &= is_p << 8;
...
point_add (felem x3, felem y3, felem z3,
           ...
           const int mixed, const smallfelem x2, const smallfelem y2,
           const smallfelem z2)
{
           ...
           const smallfelem g_pre_comp[2][16][3])
{
    ...
    for (i = (num_points ? 255 : 31); i >= 0; --i)
    {
        ...
        if (gen_mul && (i <= 31))
        {
            ...
            point_add (nq[0], nq[1], nq[2],
                       nq[0], nq[1], nq[2], 1 /* mixed */ , tmp[0], tmp[1],
                       tmp[2]);
        }
        ...
        if (num_points && (i % 5 == 0))
        {
            ...
            for (num = 0; num < num_points; ++num)
            {
                ...
                {
                    point_add (nq[0], nq[1], nq[2],
                               nq[0], nq[1], nq[2], mixed, tmp[0], tmp[1], tmp[2]);
                }
                else
                {
                    smallfelem_expand (nq[0], tmp[0]);
                                 ...
                                 BN_CTX * ctx)
{
    ...
    {
        ECerr (EC_F_EC_GFP_NISTP256_GROUP_SET_CURVE, EC_R_WRONG_CURVE_PARAMETERS);
        goto err;
    }
        ...
        BN_CTX * ctx)
{
    ...
    if (x != NULL)
    {
        ...
        {
            ECerr (EC_F_EC_GFP_NISTP256_POINT_GET_AFFINE_COORDINATES, ERR_R_BN_LIB);
            return 0;
        }
    }
    felem_mul (tmp, z1, z2);
...
static void
make_points_affine (size_t num, smallfelem points[ /* num */ ][3],
