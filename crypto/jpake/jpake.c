...
static void
JPAKE_ZKP_release (JPAKE_ZKP * zkp)
{
    BN_free (zkp->b);
    BN_free (zkp->gr);
}
...
void
JPAKE_STEP_PART_init (JPAKE_STEP_PART * p)
{
    p->gx = BN_new ();
...
void
JPAKE_STEP_PART_release (JPAKE_STEP_PART * p)
{
...
void
JPAKE_STEP1_init (JPAKE_STEP1 * s1)
{
...
void
JPAKE_STEP1_release (JPAKE_STEP1 * s1)
{
...
static void
JPAKE_CTX_init (JPAKE_CTX * ctx, const char *name,
                const char *peer_name, const BIGNUM * p,
                const BIGNUM * g, const BIGNUM * q, const BIGNUM * secret)
{
...
static void
JPAKE_CTX_release (JPAKE_CTX * ctx)
{
    BN_CTX_free (ctx->ctx);
...
JPAKE_CTX *
JPAKE_CTX_new (const char *name, const char *peer_name,
...
static void
hashlength (SHA_CTX * sha, size_t l)
{
    unsigned char b[2];
...
static void
hashstring (SHA_CTX * sha, const char *string)
{
...
static void
hashbn (SHA_CTX * sha, const BIGNUM * bn)
{
...
static void
zkp_hash (BIGNUM * h, const BIGNUM * zkpg, const JPAKE_STEP_PART * p,
 ...
 */
static void
generate_zkp (JPAKE_STEP_PART * p, const BIGNUM * x,
...
static int
verify_zkp (const JPAKE_STEP_PART * p, const BIGNUM * zkpg, JPAKE_CTX * ctx)
{
    BIGNUM *h = BN_new ();
    BIGNUM *t1 = BN_new ();
    BIGNUM *t2 = BN_new ();
...
static void
generate_step_part (JPAKE_STEP_PART * p, const BIGNUM * x,
...
int
JPAKE_STEP1_generate (JPAKE_STEP1 * send, JPAKE_CTX * ctx)
{
    genrand (ctx);
...
static int
is_legal (const BIGNUM * gx, const JPAKE_CTX * ctx)
{
    BIGNUM *t;
    int res;
...
int
JPAKE_STEP1_process (JPAKE_CTX * ctx, const JPAKE_STEP1 * received)
{
...
int
JPAKE_STEP2_generate (JPAKE_STEP2 * send, JPAKE_CTX * ctx)
{
    BIGNUM *t1 = BN_new ();
    BIGNUM *t2 = BN_new ();
...
/* gx = g^{xc + xa + xb} * xd * s */
static int
compute_key (JPAKE_CTX * ctx, const BIGNUM * gx)
{
    BIGNUM *t1 = BN_new ();
    BIGNUM *t2 = BN_new ();
    BIGNUM *t3 = BN_new ();
...
int
JPAKE_STEP2_process (JPAKE_CTX * ctx, const JPAKE_STEP2 * received)
{
    BIGNUM *t1 = BN_new ();
    BIGNUM *t2 = BN_new ();
    int ret = 0;
...
static void
quickhashbn (unsigned char *md, const BIGNUM * bn)
{
    SHA_CTX sha;
...
void
JPAKE_STEP3A_init (JPAKE_STEP3A * s3a)
{
}

int
JPAKE_STEP3A_generate (JPAKE_STEP3A * send, JPAKE_CTX * ctx)
{
...
int
JPAKE_STEP3A_process (JPAKE_CTX * ctx, const JPAKE_STEP3A * received)
{
...
void
JPAKE_STEP3A_release (JPAKE_STEP3A * s3a)
{
}

void
JPAKE_STEP3B_init (JPAKE_STEP3B * s3b)
{
}

int
JPAKE_STEP3B_generate (JPAKE_STEP3B * send, JPAKE_CTX * ctx)
{
...
int
JPAKE_STEP3B_process (JPAKE_CTX * ctx, const JPAKE_STEP3B * received)
{
...
void
JPAKE_STEP3B_release (JPAKE_STEP3B * s3b)
{
}

const BIGNUM *
JPAKE_get_shared_key (JPAKE_CTX * ctx)
{
    return ctx->key;
}
