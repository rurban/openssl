...
=head1 SYNOPSIS
 ...
 OSSL_LIB_CTX *OSSL_LIB_CTX_new_from_dispatch(UNUSED(const OSSL_CORE_HANDLE *handle),
...
useful for provider authors. The I<handle> argument is ignored, the dispatch
structure argument passed in should be the same ones as passed to a provider's
