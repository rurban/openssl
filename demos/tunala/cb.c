...
void
cb_ssl_info_set_output (FILE * fp)
{
    fp_cb_ssl_info = fp;
}
...
int
cb_ssl_verify (int ok, X509_STORE_CTX * ctx)
{
...
void
cb_ssl_verify_set_output (FILE * fp)
{
    fp_cb_ssl_verify = fp;
}

void
cb_ssl_verify_set_depth (unsigned int verify_depth)
{
    int_verify_depth = verify_depth;
}

void
cb_ssl_verify_set_level (unsigned int level)
{
    if (level < 4)
...
RSA *
cb_generate_tmp_rsa (SSL * s, int is_export, int keylength)
{
