...
unsigned int
buffer_used (buffer_t * buf)
{
    return buf->used;
}

unsigned int
buffer_unused (buffer_t * buf)
{
    return (MAX_DATA_SIZE - buf->used);
}

int
buffer_full (buffer_t * buf)
{
    return (buf->used == MAX_DATA_SIZE ? 1 : 0);
}

int
buffer_notfull (buffer_t * buf)
{
    return (buf->used < MAX_DATA_SIZE ? 1 : 0);
}
...
int
buffer_notempty (buffer_t * buf)
{
    return (buf->used > 0 ? 1 : 0);
}

unsigned long
buffer_total_in (buffer_t * buf)
{
    return buf->total_in;
}

unsigned long
buffer_total_out (buffer_t * buf)
{
...
static unsigned int
buffer_adddata (buffer_t * buf, const unsigned char *ptr, unsigned int size)
{
...
static unsigned int
buffer_tobuffer (buffer_t * to, buffer_t * from, int cap)
{
...
static unsigned int
buffer_takedata (buffer_t * buf, unsigned char *ptr, unsigned int size)
{
...
int
buffer_from_fd (buffer_t * buf, int fd)
{
...
int
buffer_to_fd (buffer_t * buf, int fd)
{
...
void
buffer_from_SSL (buffer_t * buf, SSL * ssl)
{
    int ret;
...
void
buffer_to_SSL (buffer_t * buf, SSL * ssl)
{
    int ret;
...
void
buffer_from_BIO (buffer_t * buf, BIO * bio)
{
    int ret;
...
void
buffer_to_BIO (buffer_t * buf, BIO * bio)
{
    int ret;
