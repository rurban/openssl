...
void
state_machine_close (state_machine_t * machine)
{
    if (machine->ssl)
...
buffer_t *
state_machine_get_buffer (state_machine_t * machine, sm_buffer_t type)
{
    switch (type)
    {
    case SM_CLEAN_IN:
...
SSL *
state_machine_get_SSL (state_machine_t * machine)
{
    return machine->ssl;
}

int
state_machine_set_SSL (state_machine_t * machine, SSL * ssl, int is_server)
{
    if (machine->ssl)
...
int
state_machine_churn (state_machine_t * machine)
{
    unsigned int loop;
    if (machine->ssl == NULL)
    {
...
int
state_machine_close_clean (state_machine_t * machine)
{
...
int
state_machine_close_dirty (state_machine_t * machine)
{
    buffer_close (&machine->dirty_in);
    buffer_close (&machine->dirty_out);
    buffer_close (&machine->clean_in);
