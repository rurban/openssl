...
static int tunala_item_io (tunala_selector_t * selector,
                           tunala_item_t * item);
...
static int
usage (const char *errstr, int isunknownarg)
{
    if (isunknownarg)
...
static int
err_str1 (const char *fmt, const char *str1)
{
...
static int
parse_max_tunnels (const char *s, unsigned int *maxtunnels)
{
    unsigned long l;
...
static int
parse_server_mode (const char *s, int *servermode)
{
    unsigned long l;
...
static int
parse_dh_special (const char *s, const char **dh_special)
{
...
static int
parse_verify_level (const char *s, unsigned int *verify_level)
{
    unsigned long l;
...
static int
parse_verify_depth (const char *s, unsigned int *verify_depth)
{
    unsigned long l;
    ...
    if (argc > 0)
    {
        ...
        else if (strcmp (*argv, "-out_conns") == 0)
        {
            ...
            goto next_arg;
                 ...
                 (strcmp (*argv, "-help") == 0) || (strcmp (*argv, "-?") == 0))
        {
            fprintf (stderr, "%s\n", helpstring);
            return 0;
        }
...
static int
ctx_set_cert (SSL_CTX * ctx, const char *cert, const char *key)
{
    FILE *fp = NULL;
    X509 *x509 = NULL;
    EVP_PKEY *pkey = NULL;
...
static int
ctx_set_dh (SSL_CTX * ctx, const char *dh_file, const char *dh_special)
{
    DH *dh = NULL;
    FILE *fp = NULL;
...
static SSL_CTX *
initialise_ssl_ctx (int server_mode, const char *engine_id,
                    const char *CAfile, const char *cert, const char *key,
                    const char *dcert, const char *dkey,
                    const char *cipher_list, const char *dh_file,
                    const char *dh_special, int tmp_rsa, int ctx_options,
...
static void
selector_sets_init (select_sets_t * s)
{
    s->max = 0;
    FD_ZERO (&s->reads);
    FD_ZERO (&s->sends);
...
static void
selector_init (tunala_selector_t * selector)
{
...
static void
selector_add_raw_fd (tunala_selector_t * s, int fd, int flags)
{
...
static void
selector_add_listener (tunala_selector_t * selector, int fd)
{
    selector_add_raw_fd (selector, fd, SEL_READS);
}

static void
selector_add_tunala (tunala_selector_t * s, tunala_item_t * t)
{
 ...
 * which case *newfd is populated. */
static int
selector_get_listener (tunala_selector_t * selector, int fd, int *newfd)
{
...
static int
tunala_world_make_room (tunala_world_t * world)
{
...
static int
tunala_world_new_item (tunala_world_t * world, int fd,
...
static void
tunala_world_del_item (tunala_world_t * world, unsigned int idx)
{
...
static int
tunala_item_io (tunala_selector_t * selector, tunala_item_t * item)
{
