 ...
 * DES functions
 **/
static int cipher_des_cbc_init (EVP_CIPHER_CTX * ctx,
...
static int cipher_desx_cbc_init (EVP_CIPHER_CTX * ctx,
                                 const unsigned char *key,
                                 const unsigned char *iv, int enc);
...
static int
rsaref_mod_exp (BIGNUM * r0, const BIGNUM * I, RSA * rsa)
{
...
static int
bnref_mod_exp (BIGNUM * r, const BIGNUM * a, const BIGNUM * p,
...
static int
RSAref_bn2bin (BIGNUM * from, unsigned char *to, int max)
{
    int i;
...
static BIGNUM *
RSAref_bin2bn (unsigned char *from, BIGNUM * to, int max)
{
    int i;
    BIGNUM *ret;
...
static int
RSAref_Public_ref2eay (RSArefPublicKey * from, RSA * to)
{
...
static int
RSAref_Public_eay2ref (RSA * from, R_RSA_PUBLIC_KEY * to)
{
...
#ifdef undef
static int
RSAref_Private_ref2eay (RSArefPrivateKey * from, RSA * to)
{
...
static int
RSAref_Private_eay2ref (RSA * from, R_RSA_PRIVATE_KEY * to)
{
    to->bits = BN_num_bits (from->n);
    if (!RSAref_bn2bin (from->n, to->modulus, MAX_RSA_MODULUS_LEN))
        return (0);
...
static int
rsaref_private_decrypt (int len, const unsigned char *from, unsigned char *to,
                        RSA * rsa, int padding)
{
    int i, outlen = -1;
    ...
    if (!RSAref_Private_eay2ref (rsa, &RSAkey))
        goto err;
    if ((i =
                RSAPrivateDecrypt (to, (unsigned int *) &outlen,
...
static int
rsaref_private_encrypt (int len, const unsigned char *from, unsigned char *to,
                        RSA * rsa, int padding)
{
    int i, outlen = -1;
    R_RSA_PRIVATE_KEY RSAkey;
    ...
    if (!RSAref_Private_eay2ref (rsa, &RSAkey))
        goto err;
    if ((i =
                RSAPrivateEncrypt (to, (unsigned int *) &outlen,
                                   (unsigned char *) from, len, &RSAkey)) != 0)
    {
...
static int
rsaref_public_decrypt (int len, const unsigned char *from, unsigned char *to,
                       RSA * rsa, int padding)
{
    int i, outlen = -1;
...
static int
rsaref_public_encrypt (int len, const unsigned char *from, unsigned char *to,
                       RSA * rsa, int padding)
{
    int outlen = -1;
    int i;
 ...
 **/
static int
rsaref_ciphers (ENGINE * e, const EVP_CIPHER ** cipher,
                const int **nids, int nid)
{
    int ok = 1;
    if (!cipher)
...
static int
rsaref_digests (ENGINE * e, const EVP_MD ** digest, const int **nids, int nid)
{
    int ok = 1;
    if (!digest)
    {
...
static int
cipher_des_cbc_init (EVP_CIPHER_CTX * ctx, const unsigned char *key,
                     const unsigned char *iv, int enc)
{
...
static int
cipher_des_cbc_code (EVP_CIPHER_CTX * ctx, unsigned char *out,
                     const unsigned char *in, unsigned int inl)
{
...
static int
cipher_des_cbc_clean (EVP_CIPHER_CTX * ctx)
{
...
static int
cipher_des_ede3_cbc_init (EVP_CIPHER_CTX * ctx, const unsigned char *key,
                          const unsigned char *iv, int enc)
{
...
static int
cipher_des_ede3_cbc_code (EVP_CIPHER_CTX * ctx, unsigned char *out,
                          const unsigned char *in, unsigned int inl)
{
...
static int
cipher_des_ede3_cbc_clean (EVP_CIPHER_CTX * ctx)
{
    memset (data (ctx), 0, ctx->cipher->ctx_size);
    return 1;
}
...
static int
cipher_desx_cbc_init (EVP_CIPHER_CTX * ctx, const unsigned char *key,
                      const unsigned char *iv, int enc)
{
...
static int
cipher_desx_cbc_code (EVP_CIPHER_CTX * ctx, unsigned char *out,
                      const unsigned char *in, unsigned int inl)
{
...
static int
cipher_desx_cbc_clean (EVP_CIPHER_CTX * ctx)
{
    memset (data (ctx), 0, ctx->cipher->ctx_size);
    return 1;
}
...
static int
digest_md2_init (EVP_MD_CTX * ctx)
{
    MD2Init (data (ctx));
    return 1;
}
...
static int
digest_md2_update (EVP_MD_CTX * ctx, const void *data, unsigned long count)
{
...
static int
digest_md2_final (EVP_MD_CTX * ctx, unsigned char *md)
{
...
static int
digest_md5_update (EVP_MD_CTX * ctx, const void *data, unsigned long count)
{
