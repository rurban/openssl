...
int
gost_digest_init (EVP_MD_CTX * ctx)
{
...
int
gost_digest_update (EVP_MD_CTX * ctx, const void *data, size_t count)
{
    return hash_block ((gost_hash_ctx *) ctx->md_data, data, count);
}

int
gost_digest_final (EVP_MD_CTX * ctx, unsigned char *md)
{
...
int
gost_digest_copy (EVP_MD_CTX * to, const EVP_MD_CTX * from)
{
...
int
gost_digest_cleanup (EVP_MD_CTX * ctx)
{
    if (ctx->md_data)
