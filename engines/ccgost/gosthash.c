...
/* was A_A */
static void
circle_xor8 (const byte * w, byte * k)
{
    byte buf[8];
    int i;
    memcpy (buf, w, 8);
...
static int
add_blocks (int n, byte * left, const byte * right)
{
    int i;
    int carry = 0;
    int sum;
...
static void
xor_blocks (byte * result, const byte * a, const byte * b, size_t len)
{
    size_t i;
    for (i = 0; i < len; i++)
        result[i] = a[i] ^ b[i];
 ...
 */
static int
hash_step (gost_ctx * c, byte * H, const byte * M)
{
 ...
 */
int
init_gost_hash_ctx (gost_hash_ctx * ctx, const gost_subst_block * subst_block)
{
 ...
 */
void
done_gost_hash_ctx (gost_hash_ctx * ctx)
{
 ...
 */
int
hash_block (gost_hash_ctx * ctx, const byte * block, size_t length)
{
    if (ctx->left)
    {
 ...
 */
int
finish_hash (gost_hash_ctx * ctx, byte * hashval)
{
    byte buf[32];
    byte H[32];
    byte S[32];
