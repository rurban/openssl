...
void
dump_dsa_sig (const char *message, DSA_SIG * sig)
{
    fprintf (stderr, "%s\nR=", message);
 ...
 */
DSA_SIG *
gost_do_sign (const unsigned char *dgst, int dlen, DSA * dsa)
{
 ...
 */
int
pack_sign_cp (DSA_SIG * s, int order, unsigned char *sig, size_t * siglen)
{
    *siglen = 2 * order;
    memset (sig, 0, *siglen);
...
int
gost_do_verify (const unsigned char *dgst, int dgst_len,
                DSA_SIG * sig, DSA * dsa)
{
    BIGNUM *md, *tmp = NULL;
    BIGNUM *q2 = NULL;
 ...
 */
int
fill_GOST94_params (DSA * dsa, int nid)
{
...
DSA_SIG *
unpack_cp_signature (const unsigned char *sig, size_t siglen)
{
    DSA_SIG *s;
...
BIGNUM *
getbnfrombuf (const unsigned char *buf, size_t len)
{
    while (*buf == 0 && len > 0)
    {
        buf++;
 ...
 * by zeros */
int
store_bignum (BIGNUM * bn, unsigned char *buf, int len)
{
