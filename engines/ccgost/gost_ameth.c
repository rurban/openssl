...
static ASN1_STRING *
encode_gost_algor_params (const EVP_PKEY * key)
{
 ...
 */
static int
decode_gost_algor_params (EVP_PKEY * pkey, X509_ALGOR * palg)
{
...
static int
gost_set_priv_key (EVP_PKEY * pkey, BIGNUM * priv)
{
...
BIGNUM *
gost_get0_priv_key (const EVP_PKEY * pkey)
{
    switch (EVP_PKEY_base_id (pkey))
    {
    case NID_id_GostR3410_94:
...
static int
pkey_ctrl_gost (EVP_PKEY * pkey, int op, long arg1, void *arg2)
{
    switch (op)
    {
...
static int
priv_decode_gost (EVP_PKEY * pk, PKCS8_PRIV_KEY_INFO * p8inf)
{
...
/* ----------------------------------------------------------------------*/
static int
priv_encode_gost (PKCS8_PRIV_KEY_INFO * p8, const EVP_PKEY * pk)
{
...
/* --------- printing keys --------------------------------*/
static int
print_gost_94 (BIO * out, const EVP_PKEY * pkey, int indent,
...
static int
param_print_gost94 (BIO * out, const EVP_PKEY * pkey, int indent,
                    ASN1_PCTX * pctx)
{
...
static int
pub_print_gost94 (BIO * out, const EVP_PKEY * pkey, int indent,
                  ASN1_PCTX * pctx)
{
...
static int
priv_print_gost94 (BIO * out, const EVP_PKEY * pkey, int indent,
                   ASN1_PCTX * pctx)
{
...
static int
print_gost_01 (BIO * out, const EVP_PKEY * pkey, int indent,
               ASN1_PCTX * pctx, int type)
{
    int param_nid = NID_undef;
    if (type == 2)
...
static int
param_print_gost01 (BIO * out, const EVP_PKEY * pkey, int indent,
                    ASN1_PCTX * pctx)
{
...
static int
pub_print_gost01 (BIO * out, const EVP_PKEY * pkey, int indent,
                  ASN1_PCTX * pctx)
{
...
static int
priv_print_gost01 (BIO * out, const EVP_PKEY * pkey, int indent,
                   ASN1_PCTX * pctx)
{
...
/* ---------------------------------------------------------------------*/
static int
param_missing_gost94 (const EVP_PKEY * pk)
{
...
static int
param_copy_gost94 (EVP_PKEY * to, const EVP_PKEY * from)
{
...
#define COPYBIGNUM(a,b,x) if (a->x) BN_free(a->x); a->x=BN_dup(b->x);
    ...
    COPYBIGNUM (dto, dfrom, q) COPYBIGNUM (dto, dfrom, g) if (dto->priv_key)
...
static int
param_cmp_gost94 (const EVP_PKEY * a, const EVP_PKEY * b)
{
...
static int
pub_decode_gost94 (EVP_PKEY * pk, X509_PUBKEY * pub)
{
    X509_ALGOR *palg = NULL;
...
static int
pub_encode_gost94 (X509_PUBKEY * pub, const EVP_PKEY * pk)
{
...
static int
pub_cmp_gost94 (const EVP_PKEY * a, const EVP_PKEY * b)
{
    const DSA *da = EVP_PKEY_get0 ((EVP_PKEY *) a);
    const DSA *db = EVP_PKEY_get0 ((EVP_PKEY *) b);
...
static int
pkey_size_gost (const EVP_PKEY * pk)
{
    return 64;
}

static int
pkey_bits_gost (const EVP_PKEY * pk)
{
    return 256;
}
...
static int
gost94_param_encode (const EVP_PKEY * pkey, unsigned char **pder)
{
...
static int
gost2001_param_encode (const EVP_PKEY * pkey, unsigned char **pder)
{
    int nid =
        EC_GROUP_get_curve_name (EC_KEY_get0_group
                                 (EVP_PKEY_get0 ((EVP_PKEY *) pkey)));
...
static int
gost94_param_decode (EVP_PKEY * pkey, const unsigned char **pder, int derlen)
{
...
static int
gost2001_param_decode (EVP_PKEY * pkey, const unsigned char **pder,
                       int derlen)
{
    ASN1_OBJECT *obj = NULL;
    int nid;
