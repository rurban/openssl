...
static int
pkey_gost_init (EVP_PKEY_CTX * ctx)
{
...
static int
pkey_gost_copy (EVP_PKEY_CTX * dst, EVP_PKEY_CTX * src)
{
...
static void
pkey_gost_cleanup (EVP_PKEY_CTX * ctx)
{
...
/* --------------------- control functions  ------------------------------*/
static int
pkey_gost_ctrl (EVP_PKEY_CTX * ctx, int type, int p1, void *p2)
{
...
static int
pkey_gost_ctrl94_str (EVP_PKEY_CTX * ctx, const char *type, const char *value)
{
    int param_nid = 0;
...
static int
pkey_gost_ctrl01_str (EVP_PKEY_CTX * ctx, const char *type, const char *value)
{
    int param_nid = 0;
    if (!strcmp (type, param_ctrl_string))
    {
...
static int
pkey_gost_paramgen_init (EVP_PKEY_CTX * ctx)
{
    return 1;
}

static int
pkey_gost94_paramgen (EVP_PKEY_CTX * ctx, EVP_PKEY * pkey)
{
    struct gost_pmeth_data *data = EVP_PKEY_CTX_get_data (ctx);
    DSA *dsa = NULL;
...
static int
pkey_gost94cp_keygen (EVP_PKEY_CTX * ctx, EVP_PKEY * pkey)
{
    DSA *dsa;
...
static int
pkey_gost01cp_keygen (EVP_PKEY_CTX * ctx, EVP_PKEY * pkey)
{
    EC_KEY *ec;
...
static int
pkey_gost94_cp_sign (EVP_PKEY_CTX * ctx, unsigned char *sig, size_t * siglen,
...
static int
pkey_gost01_cp_sign (EVP_PKEY_CTX * ctx, unsigned char *sig, size_t * siglen,
                     const unsigned char *tbs, size_t tbs_len)
{
    DSA_SIG *unpacked_sig = NULL;
    EVP_PKEY *pkey = EVP_PKEY_CTX_get0_pkey (ctx);
...
static int
pkey_gost94_cp_verify (EVP_PKEY_CTX * ctx, const unsigned char *sig,
...
static int
pkey_gost01_cp_verify (EVP_PKEY_CTX * ctx, const unsigned char *sig,
                       size_t siglen, const unsigned char *tbs,
                       size_t tbs_len)
{
    int ok = 0;
...
static int
pkey_gost_encrypt_init (EVP_PKEY_CTX * ctx)
{
    return 1;
}
...
/* --------------- Derive init ------------------------------------*/
static int
pkey_gost_derive_init (EVP_PKEY_CTX * ctx)
{
    return 1;
}
...
static int
pkey_gost_mac_ctrl_str (EVP_PKEY_CTX * ctx,
                        const char *type, const char *value)
{
    if (!strcmp (type, key_ctrl_string))
    {
        if (strlen (value) != 32)
        {
            GOSTerr (GOST_F_PKEY_GOST_MAC_CTRL_STR, GOST_R_INVALID_MAC_KEY_LENGTH);
            return 0;
        }
...
static int
pkey_gost_mac_signctx (EVP_PKEY_CTX * ctx, unsigned char *sig,
...
/* ----------------------------------------------------------------*/
int
register_pmeth_gost (int id, EVP_PKEY_METHOD ** pmeth, int flags)
{
