...
main (int argc, char *argv[])
{
...
#if 0 && defined(OPENSSL_NO_DYNAMIC_ENGINE)
    {
        ...
        for (t = 0; t < sizeof (tcs) / sizeof (tcs[0]); t++)
        {
            ...
            if (0 == strcmp (tcs[t].szParamSet, "id-GostR3410-94-TestParamSet"))
            {
            ...
            case G89_CNT:
                ...
                continue;
            case G89_IMIT:
                gost_mac (&ctx, 32, tcs[t].bIn, (unsigned int) tcs[t].ullLen, bTest);
    ...
    for (t = 0; t < sizeof (tcs) / sizeof (tcs[0]); t++)
    {
        ...
        if (!ENGINE_ctrl_cmd_string (impl, "CRYPT_PARAMS", tcs[t].szParamSet, 0))
        {
            fflush (NULL);
